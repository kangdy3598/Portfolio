	...
	float screenDepth, float screenNear)
{
	...
	m_FontBatch = new SpriteBatch(m_deviceContext);
	//m_Font = new SpriteFont(m_device, L"../Graphics/굴림9k.sfont");
...
void D3DClass::FrameCheck()
{
	// 프레임을 체크합니다.
	/*static float oldTime = GetTickCount();
	float nowTime = GetTickCount64();
	float dTime = nowTime - oldTime;
	...
	if (dTime >= 999)
	{
		...
		fps = (float)frmcnt * 1000 / dTime;
		frmcnt = 0;
		second++;
	}*/

	//Text(100, 10, L"FPS:%.1f/%d", fps, frmcnt);
	//Text(100, 30, L"%ds", second);
}

void D3DClass::Text(int screenWidth, int Y, float size, XMVECTOR TextColor, const TCHAR* msg, ... )
{
	...
	m_device->CreateRasterizerState(&RSDesc, &rasterizerState);
	...
	m_Font->SetLineSpacing(14.0f);
	m_Font->SetDefaultCharacter('_');
	
	int X = screenWidth / 2 - _tcslen(msg) * 7;
	m_FontBatch->Begin(SpriteSortMode_Deferred, nullptr, nullptr, nullptr, rasterizerState);
	m_Font->DrawString(m_FontBatch, buff, XMFLOAT2(X, Y), TextColor, 0.0f, (XMFLOAT2&)g_XMZero, size , SpriteEffects_None, 0.0f);
