...
{
	// 정점 및 픽셀 쉐이더를 초기화합니다.
	return InitializeShader(device, hwnd, L"../Graphics/texture.vs", L"../Graphics/texture.ps");
}
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, LPCWSTR vsFilename, LPCWSTR psFilename)
{
	...
	result = device->CreateBlendState(&blendStateDesc, &m_blendState);
	if (FAILED(result))
	{
		return false;
	}
...
void TextureShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	...
	// 삼각형을 그립니다.
	deviceContext->DrawIndexed(indexCount * 3, 0, 0);
}